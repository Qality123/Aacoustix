      ...
      <div className="entity-header" style={{ background: 'linear-gradient(135deg, #111111, #000000)' }}>
        <div className="entity-header-content">
          <div className="entity-image flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.05)' }}>
            <Flame size={32} className="sm:w-[64px] sm:h-[64px] text-accent" />
          </div>
          <div>
            <p className="text-xs uppercase tracking-widest mb-2">{greeting.icon} {greeting.text}{user?.displayName ? `, ${user.displayName}` : ''}</p>
            <h1 className="text-3xl sm:text-5xl font-bold mb-2">Trending</h1>
            <p className="text-text-secondary text-sm">Hottest tracks right now</p>
          </div>
        </div>
      </div>
      <div className="pt-2">
